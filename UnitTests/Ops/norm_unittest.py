...
class NormUnitTest:
    ...
    def forward(self, input_x_tri, input_x_tor, atol, rtol):
        ...
        self.start.record()
        output_tor = torch.norm(input_x_tor, p=self.norm.p, dim=-1, keepdim=True)
        self.end.record()
        torch.cuda.synchronize()
        self.torch_time_fwd = self.start.elapsed_time(self.end)
        
        assert torch.allclose(output_tri, output_tor.squeeze(-1), atol=atol, rtol=rtol), 'Error in forward pass'
        if self.print_tb:
            self.diff_f = (output_tri - output_tor.squeeze(-1)).abs()
    ...
    def backward(self, input_x_tri, input_x_tor, output_tri, output_tor, atol, rtol):
        ...
        self.start.record()
        output_tri.backward(g.squeeze(-1))
        self.end.record()
        torch.cuda.synchronize()
        ...
        assert torch.allclose(input_x_tri.grad, input_x_tor.grad.squeeze(-1), atol=atol, rtol=rtol), 'Error in backward pass'
        if self.print_tb:
    ...
    for D in [32, 64, 128, 256, 512, 1024]:
        for i in range(2):
            ...
            for dtype in [torch.float16, torch.float32]:
                runner = NormUnitTest(B, M, D, p, dtype, print_tb)
                runner.run()
